...
- Consumers run `npx skills add Paldom/2fa-agents` — the CLI
  ...
  users can also `gh skill install Paldom/2fa-agents <skill>
...
gh repo edit Paldom/2fa-agents --visibility public --accept-visibility-change-consequences
...
gh api repos/Paldom/2fa-agents/private-vulnerability-reporting --method PUT

# 3. Cut a versioned release (matches .claude-plugin/plugin.json) + topics
gh skill publish --tag v0.1.0
gh repo edit Paldom/2fa-agents --add-topic skills-sh
#    (gh < 2.90 has no `gh skill`; equivalent fallback:
#     gh release create v0.1.0 --generate-notes
#     gh repo edit Paldom/2fa-agents --add-topic agent-skills --add-topic skills-sh)
...
npx skills@latest add Paldom/2fa-agents --list
mkdir -p /tmp/skills-verify && cd /tmp/skills-verify
env -u DISABLE_TELEMETRY -u DO_NOT_TRACK npx skills@latest add Paldom/2fa-agents --skill '*' -a claude-code -y
...
curl -sIL https://skills.sh/Paldom/2fa-agents | head -1
```
...
(`https://skills.sh/b/Paldom/2fa-agents`) linking to the repo's
skills.sh page.
